...
# classifier = load_model('resources/MachineHuman_model_100.h5')
...
#Training Set
train_set = train_datagen.flow_from_directory('train_100',
                                             target_size=(100,100),
                                             batch_size=32,
                                             class_mode='binary')
#Validation Set
...
import numpy as np
img1 = image.load_img('validation/human/human1 (2).jpg', target_size=(100,100))
...
for i in range(columns*rows):
    #img = mpimg.imread()
    img = image.load_img('validation/'+HumanasMachine.iloc[i], target_size=(100,100))
...
for i in range(columns*rows):
    #img = mpimg.imread()
    img = image.load_img('validation/'+MachineasHuman.iloc[i], target_size=(100,100))
    fig.add_subplot(rows, columns, i+1)
    plt.imshow(img)
# plt.show()
...
for i in range(columns*rows):
    fig.add_subplot(rows, columns, i+1)
    img1 = image.load_img('test/'+test_set1.filenames[np.random.choice(range(10))], target_size=(100,100))
    img = image.img_to_array(img1)
    img = img/255
    img = np.expand_dims(img, axis=0)
    prediction = classifier.predict(img, batch_size=None,steps=1) #gives all class prob.
